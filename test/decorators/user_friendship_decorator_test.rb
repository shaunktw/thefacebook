...
class UserFriendshipDecoratorTest < Draper::TestCase
	context "#sub_message" do
		setup do
			@friend = create(:user, first_name: 'Aniket')
		end
	...
	context "with a pending user friendship" do
			...
			should "return the correct message" do
				assert_equal "Do you really want to be friends with Aniket?", @decorator.sub_message
			end
		end
		...
		should "return the correct message" do
			assert_equal "You are friends with Aniket.", @decorator.sub_message
		end
	end
	end
