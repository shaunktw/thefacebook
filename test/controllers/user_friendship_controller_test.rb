...
context "#index" do
		...
		context "when logged in" do
			setup do
				@friendship1 = create(:pending_user_friendship, user: users(:shaun), friend: create(:user, first_name: 'Pending', last_name: 'Friend'))
				@friendship2 = create(:accepted_user_friendship, user: users(:shaun), friend: create(:user, first_name: 'Active', last_name: 'Friend'))
			...
			should "display pending information on a pending friendhip" do
				assert_select "#user_friendship_#{@friendship1.id}" do
					assert_select "em", "Friendship is pending."
				end
			end

			should "display pending information on an accepted friendhip" do
			...
			context "successfully" do
				should "create two user friendship object" do
					assert_difference 'UserFriendship.count', 2 do
						post :create, user_friendship: {friend_id: users(:shaun).profile_name}
					end
				end
			end
	...
	context "when logged in"  do
		setup do
			@friend = create(:user)
			@user_friendship = create(:pending_user_friendship, user: users(:shaun), friend: @friend)
			create(:pending_user_friendship, friend: users(:shaun), user: @friend)
			sign_in users(:shaun)
		...
		should "update the state to be accepted" do
...
context "#edit" do
	...
	context "when logged in"  do
		setup do
			@friend = create(:user)
			@user_friendship  =create(:accepted_user_friendship, friend: @friend, user: users(:shaun))
			create(:accepted_user_friendship, friend: users(:shaun), user: @friend)
			sign_in users(:shaun)
